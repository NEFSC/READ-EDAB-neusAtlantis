...
# # run.dir=getwd()
# # param.dir='C:/Users/ryan.morse/Documents/GitHub/atneus_RM' #where (PRM, bgm, group data) are saved
# # # param.dir="C:/Users/ryan.morse/Documents/GitHub/atneus_RM_20180827"
# # 
# # # #linux
# # param.dir='/home/ryan/Git/atneus737e3d' # for NEUS 1.0 on new code base RM
# param.dir='/home/ryan/Git/atneus_RM'
# run.dir='/home/ryan/AtlRuns/20181110b'
# setwd(run.dir)

files=list.files(path=paste0(run.dir,runfile,'/'), pattern='.nc')
nc.str=strsplit(files, '.nc')
lncstr=nchar(nc.str)
ncbase=nc.str[which(lncstr==min(lncstr))] #get base nc file name
nc_gen    <- file.path(paste0(run.dir,runfile), paste(ncbase, '.nc', sep=""))
nc_prod   <- file.path(paste0(run.dir,runfile), paste(ncbase, 'PROD.nc', sep=""))
dietcheck <- file.path(paste0(run.dir,runfile), paste(ncbase, 'DietCheck.txt', sep=""))
yoy       <- file.path(paste0(run.dir,runfile), paste(ncbase, 'YOY.txt', sep=""))
ssb       <- file.path(paste0(run.dir,runfile), paste(ncbase, 'SSB.txt', sep=""))
if(include_catch==T){
  catchfile <- file.path(paste0(run.dir,runfile), paste(ncbase, 'CATCH.nc', sep=""))
  catchtotfile <- file.path(paste0(run.dir,runfile), paste(ncbase, 'TOTCATCH.nc', sep=""))
}

xml.files=list.files(path=paste0(run.dir,runfile,'/'), pattern='.xml')
...
prm_run <- file.path(param.dir, paste(xml.str[[i]], '.prm',sep=''))
prm_run #make sure
for (i in 1:length(xml.files)){
...
prm_biol <- file.path(param.dir, paste(xml.str[[i]], '.prm',sep=''))
prm_biol #make sure
for (i in 1:length(xml.files)){
...
fgs       <- file.path(param.dir, paste(xml.str[[i]], '.csv',sep='')) #file.path(param.dir, "NeusGroups_v15_LTLonly.csv") #unix.csv")
fgs #make sure
# bgm.files=list.files(path=param.dir, pattern='.bgm')
bgm.files="neus_tmerc_RM.bgm"
bgm       <- file.path(param.dir, bgm.files) #"neus_tmerc_RM.bgm")
...
init.file=test[[1]][3]
init = file.path(param.dir, init.file)
init # make sure
# init      <- file.path(param.dir, 'RMinit_notsohighvertmix.nc')# "RMinit_newvalues2017.nc")
init=paste(param.dir,'RMinit4_nofill_2019.nc', sep='') # dropped _FillValue to make init_growth work 20180412 (see history in cdf version for command)
...
if (check_scale_init==1){
  ...
  rm(tt, foo) # cleanup
  des.bio=readxl::read_xlsx(paste(param.dir, '/NEUS_v15_Initial_Biomass.xlsx', sep=''), col_names = T)
...
#                        MoreArgs = list(nc = nc_prod, bps = bps, fgs = fgs, prm_run = prm_run, bboxes = bboxes))
#Doesn't Work?? Ask RM. Something wrong with PB for 02112020_PrimProdDiag_2/3
dfs_noPred_prod <- Map(load_nc, select_variable = vars, select_groups = c('Diatom','PicoPhytopl'),
                       MoreArgs = list(nc = nc_prod, bps = bps, fgs = fgs, prm_run = prm_run, bboxes = bboxes))
# dfs_noPred_prod <- load_nc(select_variable = vars, select_groups = grps_noPred,nc = nc_prod,bps = bps, fgs = fgs, prm_run = prm_run, bboxes = bboxes)
...
# specmort <- file.path(run.dir, paste(ncbase, 'SpecificPredMort.txt', sep=''))
...
# filename=sapply(strsplit(as.character(run.dir), "/"), tail, 1) # grab last chars of folder
save(result, file=paste(run.dir,'/',runfile,'/',runfile, '_prepro.rdata',sep=''))
save.image(paste(run.dir,'/',runfile, '/ws.RData', sep='')) #"~/AtlRuns/20190111a/ws.RData")
