...
make_SatPhyto_files = function(in.dir,
  ...
  #Define input files
  micro.all = data.table::fread(paste0(rawdata.dir,micro.file))%>%
    select(PERIOD,SUBAREA,all_of(stat.var))%>%
  ...
  nanopico.all =data.table::fread(paste0(rawdata.dir,nanopico.file))%>%
    select(PERIOD,SUBAREA,all_of(stat.var))%>%
    tidyr::separate(PERIOD,c('DURATION','DATE.START','DATE.END'),sep = '_')%>%
    mutate(DATE = as.Date(DATE.END,'%Y%m%d'),
           DOY = format(DATE,'%j'),
  ...
  # for(i in 1:2){
    ...
    micro.year = micro.all %>%
      filter(YEAR == years[i])%>%
      rename('STAT' = all_of(stat.var))
    
    nanopico.year = nanopico.all %>%
      filter(YEAR == years[i])%>%
      rename('STAT' = all_of(stat.var))
    ...
    micro.mat = micro.year %>%
      as.data.frame()%>%
      arrange(SUBAREA,DATE)%>%
      mutate(STAT.N = STAT*chl.conv)%>%
      select(SUBAREA,DATE,STAT.N)%>%
      tidyr::spread(DATE,STAT.N)%>%
      select(-SUBAREA)
    
    nanopico.mat = nanopico.year %>%
      as.data.frame()%>%
      arrange(SUBAREA,DATE)%>%
      mutate(STAT.N = STAT*chl.conv)%>%
      select(SUBAREA,DATE,STAT.N)%>%
    ...
    prod.ls = list(micro.mat,nanopico.mat)
