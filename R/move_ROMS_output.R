...
new.out.dir = 'C:/Users/joseph.caracappa/Documents/Atlantis/ROMS_COBALT/ROMS_COBALT_Output/'
...
for(yr in 1:length(years)){
  ...
  statevars.file = all.files[which(sapply(all.files,function(x) any(strsplit(x,'[_.]+')[[1]]=='statevars') & all(strsplit(x,'[_.]+')[[1]] != 'ltl')))]
  ltlvars.file = all.files[which(sapply(all.files, function(x) any(strsplit(x,'[_.]+')[[1]] == 'ltl')))]
  nutvars.file = all.files[which(sapply(all.files, function(x) any(strsplit(x,'[_.]+')[[1]] == 'nutvars')))]
