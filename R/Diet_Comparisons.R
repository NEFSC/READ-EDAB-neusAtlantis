## Function to make diet comparisons (predators and prey) for a given set of species across model runs
...
## diet.method = 'diet.topn' takes the top 'n' prey from a predators diet
  ...
  prey.groups.full = fgs2$LongName[match(prey.groups,fgs2$Code)]
  
  plots.all = list()
  biomass.ls = list()
  
  run.names = unique(c(runfile,other.runs))
  ...
  if(make_biomasscomp_plots == T){
    ...
    ggsave(paste0(runfile,' Biomass Diagnostics.png'),biomass.plot, width = 18, height = 10, units = 'in',dpi = 300)
  ...
  if(make_dietcomp_plots == T){
    ...
    ggsave(paste0(runfile,' Diet Diagnostics.png'),diet.plot,width = 18, height = 18, units = 'in', dpi = 300)
...
# fg.file = 'C:/Users/joseph.caracappa/Documents/GitHub/neus-atlantis/NeusGroups_v15_unix_RM.csv' 
# # run.names = c('atneus_v15_01272020', 'atneus_v15_01272020_NewBivalvePred' ,'02042020_NewBivalvePred_2' ,'02042020_NewBivalvePred_3') 
# run.names = c('atneus_v15_01272020','02042020_NewBivalvePred_3','02062020_PrimProdDiag_1','0206202_PrimProdDiag_KN')
# pred.groups = NA 
# prey.groups = c('PL') 
# make_dietcomp_plots = F 
# make_biomasscomp_plots = T 
# plot.title = 'Diatoms_V4' 
# diet.method = 'percent' 
# diet.fract = 0.9 
# tstart = 20 
# log.biomass = T 
