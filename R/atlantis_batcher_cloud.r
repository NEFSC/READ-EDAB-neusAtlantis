...
atlantis_batcher = function(batcherFilename, userName, CHECK_TIME_INTERVAL = 30, NUM_TO_RUN = 3, CONTAINER_TYPE = 'podman',param.dir,output.dir) {
  ...
  container_ps_output <- system(paste0('sudo ',CONTAINER_TYPE, " ps"), intern = TRUE)
  ...
  for (n in 1:numContainersToCreate) {
    ...
    system(paste0("sudo mkdir -p ",paste0('"',output.dir,folders[n],'"')))
    ...
    print(paste0(CONTAINER_TYPE," run -d --name ",  containerName, " --rm --mount \"type=bind,src=",param.dir,",dst=/app/model\" --mount \"type=bind,src=",outputFolder,"/,","dst=/app/model/output/\" atlantis:6536"))
    run <- paste0('sudo ',CONTAINER_TYPE," run -d --name ",  containerName, " --rm --mount \"type=bind,src=",param.dir,",dst=/app/model\" --mount \"type=bind,src=",outputFolder,"/,","dst=/app/model/output/\" atlantis:6536")
    system(run)
    ...
    if ((length(startedContainers) == 0) && (n >= numRuns)) {
      ...
      if (numToSetup > 0) {
        ...
        for (i in n:endRun) {
          ...
          system(paste0("sudo mkdir -p ",paste0('"',output.dir,folders[i],'"')))
