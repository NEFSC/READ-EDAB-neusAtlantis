...
real.spp = unique(realBiomass$Code)

realBiomass.guild = realBiomass |> 
  dplyr::left_join(guild2spp) |> 
  dplyr::group_by(Guild, YEAR) |> 
  dplyr::summarise(value = sum(value,na.rm=T)) |> 
  dplyr::rename(Code = 'Guild') |> 
  dplyr::mutate(isFishedSpecies =ifelse(Code %in% c('Mammals','Other','Shrimp','Zooplankton'),F,T),
                Species = Code,
                variable = 'biomass')
...
#make fake fgs file
fgs.guild.dat = data.frame(Code = sort(unique(guild2spp$Guild))) |> 
  dplyr::mutate(Index = (1:n())-1,
                Name = Code,
                IsTurnedOn =1)
write.csv(fgs.guild.dat,fgs.guild.file,row.names =F)
...
get_diag_guild = function(day.start, day.end){
  ...
  new.biomass = biomass |> 
    ...
    tidyr::pivot_wider(names_from = Guild, values_from = Biomass) |> 
    dplyr::rename(time = 'Time')
  
  new.biomass.reasonable =  biomass |> 
    dplyr::filter(Time <= day.end) |> 
    tidyr::pivot_longer(cols = -Time, names_to = 'Code', values_to = 'Biomass') |> 
    dplyr::filter(Code %in% real.spp) |> 
    dplyr::left_join(guild2spp) |> 
    dplyr::filter(!is.na(Guild)) |> 
    dplyr::group_by(Time, Guild) |> 
    dplyr::summarise(Biomass = sum(Biomass)) |> 
  ...
  stabilty.guild = atlantisdiagnostics::diag_stability(fgs = fgs.guild.file,
  ...
  reasonable.guild = atlantisdiagnostics::diag_reasonability(fgs = fgs.guild.file,
  ...
  file.remove(c(bio.temp.name,bio.reasonable.temp.name,catch.temp.name)
  )
  return(data.out)
}
