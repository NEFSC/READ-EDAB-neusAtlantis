...
bio.run.stats = readRDS(paste0(data.dir,'recovery_stats_fspike_combined.rds')) %>%
  ...
  arrange(LongName)%>%
  mutate(db.t15.tmin = (db.t15 - db.tmin)/db.tmin)%>%
  mutate(db.t15.tmin = ifelse(!is.finite(db.t15.tmin),-1,db.t15.tmin))

bio.run.stats[which(bio.run.stats == Inf,arr.ind = T)] = 0

spp.min = bio.run.stats %>%
  select(Code,LongName,scalar,db.t15.tmin)%>%
  mutate(is.zero = ifelse(db.t15.tmin == 0,T,F))%>%
...
bio.run.sort = bio.run.stats2 %>%
  select(Code,LongName,Guild,scalar,db.t15.tmin)%>%
  group_by(Code)%>%
  mutate(min.prop = db.t15.tmin == min(db.t15.tmin))%>%
  filter(min.prop == T)%>%
  ungroup()%>%
  distinct(Code,db.t15.tmin,.keep_all = T)%>%
  arrange(Guild,db.t15.tmin)%>%
...
ggplot(bio.run.stats2, aes(color= factor(scalar),
                           ...
                           x = db.t15.tmin,xend = 0))+
  geom_segment(color = 'grey70')+
  # geom_point(size = 4,shape = 108,alpha = 0.6)+
  geom_errorbar(aes(ymin = plot.order-.4,ymax = plot.order+.4, x = db.t15.tmin),linewidth = 1)+
  ...
  xlab('Recoved proportion after 15 years (Relative to Disturbance')+
  ylab('')+
  theme_bw()+
  theme(panel.grid.minor =element_blank(),
        legend.position = 'bottom',
        axis.text.y = element_text(color = name.col$plot.color ))
ggsave(paste0(figure.dir,'Figure_8c_alt_Recovery_Prop_15yr.png'),width = 7, height = 8, units = 'in',dpi = 300)

recovered.prop = bio.run.stats2 %>%
  select(Code,LongName,Guild,scalar,db.t15.tmin)%>%
  group_by(Guild,scalar)%>%
  summarise(db.t15.tmin = mean(db.t15.tmin))%>%
  tidyr::spread(scalar,db.t15.tmin)

write.csv(recovered.prop,'Z:/Shared_Data/fishing_sensitivity_manuscript/tables/table_S1_recovered_proportion.csv',row.names =F)  
