...
bio.run.stats = readRDS(paste0(data.dir,'recovery_stats_fspike_combined.rds')) %>%
  ...
  left_join(fgs)%>%
  left_join(multi.spp)%>%
  filter(!is.na(recovery.5)& IsMulti ==0)%>%
...
ggplot(bio.run.stats2, aes(color= factor(scalar),
  ...
  geom_errorbar(aes(ymin = plot.order-.4,ymax = plot.order+.4, x = db.t5),linewidth = 1)+
...
ggsave(paste0(figure.dir,'Figure_8_alt_Recovery_Prop_5yr.png'),width = 7, height = 8, units = 'in',dpi = 300)
# 
# bio.run.stats2 = bio.run.stats2 %>%
#   group_by(Guild)%>%
