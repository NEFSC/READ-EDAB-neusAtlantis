...
system(paste0('sudo mkdir ',proj.dir,'currentVersion/CatchFiles/',batch.prefix))
...
for(i in 1:nrow(scenario.combs)){
  ...
  new.catch.names[i] = new.catch.name
    ...
    make_catch_spike_projected(
      proj.dir = proj.dir,
      original_catch_file = paste0(proj.dir,"currentVersion/CatchFiles/total_catch_project_mean_20yrs.ts"),
      fgs.file = paste0(proj.dir,'currentVersion/neus_groups.csv'),
      group.names =scenario.combs$Code[i],
      new_catch_file = paste0(proj.dir,new.catch.names[i]),
      setup.filename = paste0(proj.dir,'Setup_Files/',new.catch.names[i],'.csv'),
      spike.start = 19724,
