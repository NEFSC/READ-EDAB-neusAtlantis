...
make_catch_file_projected_mean = function(fgs.file,original_catch_file,start.time,end.time,duration,overwrite =F,new_catch_file){
...
make_catch_file_projected_mean(
  ...
  duration = 100*365,
  new_catch_file = here::here('currentVersion','CatchFiles','total_catch_project_mean_100yrs.ts'),
  overwrite = F
)
