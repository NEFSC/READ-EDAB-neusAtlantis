#' Scale a species forced catch time
#'
#' Scales time series (in total_catch.ts) of an individual species
#'
#' @param speciesCode Character vector. The functional group code of the species to scale
#' @param scalar Numeric. The value by which you want to multiply the time series (Default = 1)
#' @param outFile Character. The name of the output file (without the extension)
#' 
#' @Section Info
#' 
#' The time series catch file resides in the curentVersion/CatchFiles folder. 
#' The output file will be created in the same location
#' 
#' Currently the scalar is a global scalar applied to all species codes
#'

scale_catch_ts <- function(speciesCode,scalar=1,outFile) {
  
  # input and utput files
  catchFile <- here::here("currentVersion/CatchFiles/total_catch.ts")
