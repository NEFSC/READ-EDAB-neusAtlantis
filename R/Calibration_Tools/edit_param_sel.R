#Function to change the sel_XXX parameters in at_harvest.prm
...
edit_param_sel = function(harvest.file, Code, Fleet,fleets.file,Value,overwrite,new.file.name){
  
  fisheries = read.csv(fleets.file,as.is =T)
  
  harvest.lines = readLines(harvest.file)
  
  sel.name = paste0('sel_',Code)
  which.sel = grep(sel.name,harvest.lines)
  
  which.fleet = which(fisheries$Code == Fleet)
  
  orig.vals = strsplit(harvest.lines[which.sel+1],' |\t')[[1]]
  
  new.vals = orig.vals
  new.vals[which.fleet] = Value
  new.vals=paste(new.vals,collapse = ' ')
  
  harvest.lines[which.sel+1] = new.vals
