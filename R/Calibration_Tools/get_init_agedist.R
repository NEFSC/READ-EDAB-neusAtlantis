...
get_init_agedist = function(init.file,fgs.file,group.name){
  ...
  long.name = fgs$Name[which(fgs$Code == group.name)]
  full.name = fgs$LongName[which(fgs$Code == group.name)]
  
  n.age = fgs$NumCohorts[which(fgs$Code == group.name)]
  
  num.names = paste0(long.name,1:n.age,'_Nums')
  sn.names = paste0(long.name,1:n.age,'_StructN')
  rn.names = paste0(long.name,1:n.age,'_ResN')
  
  var.names = length(num.names)
  
  init.nc = nc_open(init.file)
  out.df = data.frame(age = 1:n.age,nums = NA, SN = NA, RN = NA, totN = NA,biomass = NA)
