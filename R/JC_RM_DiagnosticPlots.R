...
# run.dir='/home/ryan/AtlRuns/20170503b'
# #run.dir='/media/ryan/TOSHIBA EXT/1 RM/10 ATLANTIS transfer/20170413'
# setwd(run.dir)
# 
...
# 
filename=sapply(strsplit(as.character(runfile), "/"), tail, 1) # grab last chars of folder
# #
...
#   get(ls()[ls() != "fileName"])
# }
# prepr=list.files(run.dir, pattern=".rdata") # get name
...
# nc_gen    <- file.path(run.dir, paste(ncbase, '.nc', sep=""))
# nc_prod   <- file.path(run.dir, paste(ncbase, 'PROD.nc', sep=""))
# dietcheck <- file.path(run.dir, paste(ncbase, 'DietCheck.txt', sep=""))
# yoy       <- file.path(run.dir, paste(ncbase, 'YOY.txt', sep=""))
# ssb       <- file.path(run.dir, paste(ncbase, 'SSB.txt', sep=""))
...
# bgm file
bgm       <- file.path(param.dir, "neus_tmerc_RM2.bgm") #30_v15.bgm")
...
init_length=read.csv(file=paste(param.dir, '/vertebrate_init_length_cm.csv', sep=''), header = T, stringsAsFactors = F)
...
for (ii in 1:10){
  ...
  plot=plot_line(test)
  update_labels(p = plot, labels = list(x = "Time [years]", y = "Numbers"))
  ggsave(paste(filename," Age-",ii, " Cohort timeseries.png", sep=''), width=20, height=17, dpi=96)
}
...
if (make_LTL_plots==1){
  ...
  biom=read.table(paste(param.dir, '/R/atneus_v10_newcodebaseBiomIndx.txt', sep=''), header=T) # v1.0 on new codebase
  bio1=read.table(paste(param.dir, '/R/neusDynEffort_Base_Effort_BiomIndx.txt', sep=''), header=T) # v1.0 on old codebase
  bio2=read.table(paste(run.dir,runfile, '/', ncbase, 'BiomIndx.txt', sep=''), header=T) # v1.5 run
  phyto=read.table(paste(param.dir, '/R/phytoplankton_timeseries_biomass_tonnes_1998_2016.csv', sep=''),header=T, sep=',')
  zoo=read.table(paste(param.dir, '/R/Zooplankton_total_biomass_tonnes_N_20yrs.csv', sep=''), header =T, sep=',')
