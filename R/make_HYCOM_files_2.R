#' Converts raw HYCOM-formatted output into the form used by hydroconstruct
#' 
...
#' 
#' @param hycom.dir String. Defines path to ROMS output. All .nc files within path must be ROMS output
#' @param hycom.prefix String. ROMS output prefix to pattern match
...
hycom.dir = 'C:/Users/joseph.caracappa/Documents/HYCOM/Data/2010/'
# hycom.dir = 'D:/NWA/1980/'
hycom.prefix = 'GLBv0.08_expt_53.X_*'
hycom.files = list.files(hycom.dir,hycom.prefix)
# out.dir = 'D:/OUtput/1980/'
out.dir = 'C:/Users/joseph.caracappa/Documents/Atlantis/HYCOM/Post_Processed_Output/'
...
name.out = 'hycom_post_'
make.hflux = T
make.physvars = T


# hycom.dir =local.dir
# hycom.prefix = 'RM_NWA-SZ.HCob05T_avg_'
# out.dir = paste0(local.output.dir,dir.names[yr],'/')
# name.out = 'roms_cobalt_'

make_ROMS_files = function(hycom.dir,
                           hycom.prefix,
                           hycom.files,
                           out.dir,
                           dz.file,
  ...
  date_from_file = function(file){
    x = strsplit(file, paste0(hycom.prefix,'|.nc'))[[1]][2]
    return(x)
  ...
  # Read in ROMS Output  -----------------------------------------------------
  # setwd(hycom.dir)
  # hycom.files = dir(hycom.dir,pattern = hycom.prefix)
  ...
  file_db_ls = list()
  for(f in 1:length(hycom.files)){
    file.nc = ncdf4::nc_open(paste0(hycom.dir,hycom.files[f]))
    times = file.nc$dim$time$vals*60*60
    file_db_ls[[f]] = dplyr::tibble(fullname = hycom.files[f],
                                    # date = as.Date(date_from_file(hycom.files[f])),
                                    ocean_time = as.POSIXct(times,origin = '2000-01-01 00:00:00',tz = 'UTC'))
  ...
  roms_ll_rho<- angstroms::romscoords(paste0(hycom.dir,hycom.files[[1]]), transpose = TRUE, spatial = c("lon_rho", "lat_rho"))
  ...
  bgm_lstXY = sp::coordinates(roms_face_rho)
  ...
  p1 = raster::cellFromXY(roms_ll_rho,cbind(bgmXY[,1],bgmXY[,3]))
  p2 = raster::cellFromXY(roms_ll_rho,cbind(bgmXY[,2],bgmXY[,4]))
  ...
  if(make.hflux){
    ...
    ind_face_u = raster::cellFromLine(angstroms::romsdata(paste0(hycom.dir,hycom.files[1]),'u'),roms_face_rho)
    ind_face_v = raster::cellFromLine(angstroms::romsdata(paste0(hycom.dir,hycom.files[1]),'v'),roms_face_rho)
    
    # creates table of face cell index for u and v points
    face_roms_vindex <- dplyr::tibble(face = roms_face_rho$label[rep(seq_len(nrow(roms_face_rho)), lengths(ind_face_v))], 
                               cell = unlist(ind_face_v))
    face_roms_uindex <- dplyr::tibble(face = roms_face_rho$label[rep(seq_len(nrow(roms_face_rho)), lengths(ind_face_u))], 
  ...
  h <- readAll(raster(paste0(hycom.dir,hycom.files[1]), varname = "h", ncdf=T))
  ...
  if(any(make.ltlvars,make.physvars,make.nutvars)){
  ...
  # tic()
    ...
    # roms.nc = nc_open(paste0(hycom.dir,roms_file))
    ...
    if(make.hflux){
      r_u <- set_indextent(brick(paste0(hycom.dir,roms_file), varname = "u", lvar = 4, level = level, ncdf=T))
      r_v <- set_indextent(brick(paste0(hycom.dir,roms_file), varname = "v", lvar = 4, level = level, ncdf=T))
    }
    if(make.physvars){
      r_w <- set_indextent(brick(paste0(hycom.dir,roms_file), varname = "w", lvar = 4, level = level, ncdf=T))
      r_temp <- set_indextent(brick(paste0(hycom.dir,roms_file), varname = "temp", lvar = 4, level = level, ncdf=T))
      r_salt <- set_indextent(brick(paste0(hycom.dir,roms_file), varname = "salt", lvar = 4, level = level, ncdf=T))
    }
    
    #COBALT PARAMS
    r_rho <- set_indextent(brick(paste0(hycom.dir,roms_file), varname = "rho", lvar = 4, level = level, ncdf=T)) #Fluid Density Anomaly
    
    if(make.ltlvars){
      # r_ndi <- set_indextent(brick(paste0(hycom.dir,roms_file), varname = "ndi", lvar = 4, level = level, ncdf=T)) #Diazotroph N
      r_nlg <- set_indextent(brick(paste0(hycom.dir,roms_file), varname = "nlg", lvar = 4, level = level, ncdf=T)) #Large Phyto N
      r_nlgz <- set_indextent(brick(paste0(hycom.dir,roms_file), varname = "nlgz", lvar = 4, level = level, ncdf=T)) #Large Zoo N
      r_nmdz <- set_indextent(brick(paste0(hycom.dir,roms_file), varname = "nmdz", lvar = 4, level = level, ncdf=T)) #Med Zoo N
      r_nsm <- set_indextent(brick(paste0(hycom.dir,roms_file), varname = "nsm", lvar = 4, level = level, ncdf=T)) #Small Phtyo N
      r_nsmz <- set_indextent(brick(paste0(hycom.dir,roms_file), varname = "nsmz", lvar = 4, level = level, ncdf=T)) #Small Zoo N
      r_silg <- set_indextent(brick(paste0(hycom.dir,roms_file), varname = "silg", lvar = 4, level = level, ncdf=T)) #Large Phyto Si
      r_nbact <- set_indextent(brick(paste0(hycom.dir,roms_file), varname = "nbact", lvar = 4, level = level, ncdf=T)) #Bacterial N
    }
    
    if(make.nutvars){
      r_nh4 = set_indextent(brick(paste0(hycom.dir,roms_file),varname= 'nh4',lvar = 4, level = level, ncdf4 = T)) #Ammonia
      r_no3 = set_indextent(brick(paste0(hycom.dir,roms_file),varname= 'no3',lvar = 4, level = level, ncdf4 = T)) #Nitrate
      r_o2 = set_indextent(brick(paste0(hycom.dir,roms_file),varname= 'o2',lvar = 4, level = level, ncdf4 = T)) #Oxygen
      r_sio4 = set_indextent(brick(paste0(hycom.dir,roms_file),varname= 'sio4',lvar = 4, level = level, ncdf4 = T)) #Silicate
    }
  ...
  # load(paste0(hycom.dir,'Test Dump.R'))
  ...
  # 
  # # ocean_time=ncdf4::ncvar_get(ncdf4::nc_open(hycom.files[1]), varid='ocean_time') 
  # ocean_time = sapply(hycom.files,function(x) ncvar_get(nc_open(x),varid='ocean_time'))
  ...
  # t_tot = seq(ocean_time[1],by = dt, length.out = length(hycom.files))
...
# Roms2Hydro(hycom.dir,hycom.prefix,out.dir,name.out)
