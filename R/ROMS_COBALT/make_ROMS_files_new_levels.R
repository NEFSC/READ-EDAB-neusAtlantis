...
make_ROMS_files = function(roms.dir,
                           roms.prefix,
                           roms.files,
                           out.dir,
                           dz.file,
  ...
  if(make.hflux){
    ...
    ind_face_u = raster::cellFromLine(angstroms::romsdata(paste0(roms.dir,roms.files[1]),'u'),roms_face_rho)
    ind_face_v = raster::cellFromLine(angstroms::romsdata(paste0(roms.dir,roms.files[1]),'v'),roms_face_rho)
  ...
  atlantis_depths <- -rev(cumsum(c(0, rev(rbgm::build_dz(-max_depth, zlayers = c(-500, -300, -120, -10, 0))))))
  ...
  # tic()
    ...
    if(make.hflux){
      r_u <- set_indextent(brick(paste0(roms.dir,roms_file), varname = "u", lvar = 4, level = level, ncdf=T))
      r_v <- set_indextent(brick(paste0(roms.dir,roms_file), varname = "v", lvar = 4, level = level, ncdf=T))
    }
    if(make.physvars){
    ...
    if(make.ltlvars){
      ...
      r_nsm <- set_indextent(brick(paste0(roms.dir,roms_file), varname = "nsm", lvar = 4, level = level, ncdf=T)) #Small Phtyo N
      r_nsmz <- set_indextent(brick(paste0(roms.dir,roms_file), varname = "nsmz", lvar = 4, level = level, ncdf=T)) #Small Zoo N
      r_silg <- set_indextent(brick(paste0(roms.dir,roms_file), varname = "silg", lvar = 4, level = level, ncdf=T)) #Large Phyto Si
      r_nbact <- set_indextent(brick(paste0(roms.dir,roms_file), varname = "nbact", lvar = 4, level = level, ncdf=T)) #Bacterial N
    }
    
    if(make.nutvars){
      r_nh4 = set_indextent(brick(paste0(roms.dir,roms_file),varname= 'nh4',lvar = 4, level = level, ncdf4 = T)) #Ammonia
      r_no3 = set_indextent(brick(paste0(roms.dir,roms_file),varname= 'no3',lvar = 4, level = level, ncdf4 = T)) #Nitrate
      r_o2 = set_indextent(brick(paste0(roms.dir,roms_file),varname= 'o2',lvar = 4, level = level, ncdf4 = T)) #Oxygen
      r_sio4 = set_indextent(brick(paste0(roms.dir,roms_file),varname= 'sio4',lvar = 4, level = level, ncdf4 = T)) #Silicate
    }
  ...
  # 
  # # ocean_time=ncdf4::ncvar_get(ncdf4::nc_open(roms.files[1]), varid='ocean_time') 
  # ocean_time = sapply(roms.files,function(x) ncvar_get(nc_open(x),varid='ocean_time'))
  ...
  # t_tot = seq(ocean_time[1],by = dt, length.out = length(roms.files))
