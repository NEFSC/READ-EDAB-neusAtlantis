...
Roms2Hydro = function(roms.dir,roms.prefix,out.dir,name.out){
  ...
  box_props <- box_props_cob <- box_props_nut <- box_props_don <- face_props <- face_props_sum <- vector("list", nrow(file_db))
  i_timeslice <- 1
  ...
  # tic()
    ...
    #COBALT PARAMS - Nutrients
    r_nh4 = set_indextent(brick(roms_file,varname= 'nh4',lvar = 4, level = level, ncdf4 = T)) #Ammonia
    r_no3 = set_indextent(brick(roms_file,varname= 'no3',lvar = 4, level = level, ncdf4 = T)) #Nitrate
    r_o2 = set_indextent(brick(roms_file,varname= 'o2',lvar = 4, level = level, ncdf4 = T)) #Oxygen
    r_sio4 = set_indextent(brick(roms_file,varname= 'sio4',lvar = 4, level = level, ncdf4 = T)) #Silicate
    
    #COBALT PARAMS - Detritus
    # r_sidet = set_indextent(brick(roms_file,varname= 'sidet',lvar = 4, level = level, ncdf4 = T)) #Detrital Silica
    # r_sldon = set_indextent(brick(roms_file,varname= 'sldon',lvar = 4, level = level, ncdf4 = T)) #Semilabile DON
    # r_srdon = set_indextent(brick(roms_file,varname= 'srdon',lvar = 4, level = level, ncdf4 = T)) #semirefractory DON
    # r_ldon = set_indextent(brick(roms_file,varname= 'ldon',lvar = 4, level = level, ncdf4 = T)) #Labile DON
    # 
    # tic()
    ...
    #COBALT PARAMS - DON (mol/m3)
    # box_z_index$sidet = extract_at_level(readAll(r_sidet), box_cell)*box_z_index$rho; rm(r_sidet)
    # box_z_index$sldon = extract_at_level(readAll(r_sldon), box_cell)*box_z_index$rho; rm(r_sldon)
    # box_z_index$srdon = extract_at_level(readAll(r_srdon), box_cell)*box_z_index$rho; rm(r_srdon)
    # box_z_index$ldon = extract_at_level(readAll(r_ldon), box_cell)*box_z_index$rho; rm(r_ldon)
    # # toc()
    ...
    #COBALT-DON
    # box_props_don[[i_timeslice]] <- box_z_index2 %>% group_by(atlantis_level,.bx0) %>%
    #   summarize(rho = mean(rho,na.rm=T), sidet = mean(sidet,na.rm=T), sldon = mean(sldon,na.rm=T),srdon = mean(srdon,na.rm=T),ldon = mean(ldon,na.rm=T))%>%
    #   complete(atlantis_level,.bx0)
    # box_props_don[[i_timeslice]]$band_level = file_db$band_level[i_timeslice]
    # 
  ...
  try(save(box_props_cob,box_props_nut,file = paste0(out.dir,'post-loop dump.R')))
  ...
  # box_props_don <- bind_rows(box_props_don)
  ...
  # 
  box_props_cob[which(box_props_cob$.bx0==23 | box_props_cob$.bx0==24), c('rho','ndi','nlg','nlgz','nmdz','nsm','nsmz','silg','nbact')]=-999 #islands
  box_props_cob[is.na(box_props_cob$rho),c('rho','ndi','nlg','nlgz','nmdz','nsm','nsmz','silg','nbact')]=-999 # change NA to fill value
  
  box_props_nut[which(box_props_nut$.bx0==23 | box_props_nut$.bx0==24), c('rho','nh4','no3','o2','sio4')]=-999 #islands
  box_props_nut[is.na(box_props_nut$rho),c('rho','nh4','no3','o2','sio4')]=-999 # change NA to fill value
  
  # box_props_don[which(box_props_don$.bx0==23 | box_props_don$.bx0==24), c('rho','sidet','sldon','srdon','ldon')]=-999 #islands
  # box_props_don[is.na(box_props_don$rho),c('rho','sidet','sldon','srdon','ldon')]=-999 # change NA to fill value
  # 
  ...
  nboxes=length(unique(box_props_cob$.bx0)) #30
  ntimes=length(unique(box_props_cob$band_level))
  nlevel=length(unique(box_props_cob$atlantis_level))
  atl.level=unique(box_props_cob$atlantis_level)
  ...
  ##COBALT - Nutrient vars
  nh4 = no3 = o2 = sio4 = array(NA,dim = c(nlevel,nboxes,ntimes))
  #COBALT - DON vars
  # sidet = sldon = srdon = ldon =  array(NA,dim = c(nlevel,nboxes,ntimes))
  
  for (i in 1:length(box_props_cob$rho)){
    if (i %% 5000 ==0) print(paste('progress:',i/length(box_props_cob$rho)*100, '%'))
    #time
    j=box_props_cob$band_level[i]
    ### box NOTE added 1 because index cannot be 0, must remove later
    k=(box_props_cob$.bx0[i])+1 
    # depth
    l=box_props_cob$atlantis_level[i]
    ...
    #cobalt- DON
    # sidet[l,k,j] = box_props_don$sidet[i]
    # sldon[l,k,j] = box_props_don$sldon[i]
    # srdon[l,k,j] = box_props_don$srdon[i]
    # ldon[l,k,j] = box_props_don$ldon[i]
  }
  ...
  # save('sidet','sldon','srdon','ldon',file = paste0(out.dir,name.out,'don_',year,'.R'))
  ...
  ###FOR COBALT DON VARS
  ### For COBALT LTL variables
  # filename=paste0(out.dir,name.out,'don_2hydro.nc')
  # 
  # #define dimensions
  # timedim=ncdim_def("time", "", 1:length(t_tot), unlim=T, create_dimvar = F) #as.double(t_tot)
  # leveldim=ncdim_def("level", "", 1:nlevel, create_dimvar = F)
  ...
  # var.box=ncvar_def("boxes", "", boxesdim, longname="Box IDs", prec='integer')
  # var.lev=ncvar_def("level","",leveldim,longname="layer index; 1=near surface; positice=down" ,prec="integer")
  # var.sidet=ncvar_def('sidet','mol / m^3',list(leveldim,boxesdim,timedim),-999,longname = 'Silicon DON',prec='float')
  # var.sldon=ncvar_def('sldon','mol / m^3',list(leveldim,boxesdim,timedim),-999,longname = 'Semilabile DON',prec='float')
  # var.srdon=ncvar_def('srdon','mol / m^3',list(leveldim,boxesdim,timedim),-999,longname = 'Semirefractory DON',prec='float')
  # var.ldon=ncvar_def('ldon','mol / m^3',list(leveldim,boxesdim,timedim),-999,longname = 'Labile DON',prec='float')
  # 
  # nc_varfile = nc_create(filename, list(var.time,var.box,var.lev,
  #                                       var.sidet,var.sldon,var.srdon,var.ldon))
  # 
  # #assign global attributes to file
  # ncatt_put(nc_varfile,0,"title","Box averaged properties file, NEUS")
  # ncatt_put(nc_varfile,0,"geometry","neus_tmerc_RM.bgm")
  ...
  # 
  # #assign variables to file
  # ncvar_put(nc_varfile,var.sidet,sidet,count = c(nlevel,nboxes,ntimes))
  # ncvar_put(nc_varfile,var.sldon,sldon,count = c(nlevel,nboxes,ntimes))
  # ncvar_put(nc_varfile,var.srdon,srdon,count = c(nlevel,nboxes,ntimes))
  # ncvar_put(nc_varfile,var.ldon,ldon,count = c(nlevel,nboxes,ntimes))
  # ncvar_put(nc_varfile,var.time,t_tot,verbose = T)
  # ncvar_put(nc_varfile,var.lev,atl.level)
  # ncvar_put(nc_varfile,var.box,box.boxes)
  # 
