...
* If the trends do not look similar the mean of the first/last n years of the stocksmart time series is used.
...
Each function used to process a data stream has the option to save the processed data as an rds file. These data files are then read in to `create_neus_catch_data.r` where the stocksmart data is analysed for "missing" data. The method for imputing missing data is defined in `functionalGroupNames_catchsource.csv`
...
```{r nafons, echo = F}
knitr::include_graphics("../data/nafo_ns_catch.png")
```
...
* NAFO (Foreign) data uses live weight. Need to use comlandr meat and live weight for shellfish to scale foreign data for certain species
...
* Some species in NAFO can npt be accurately mapped to Atlantis groups
