...
Podman can not be used on centos7 in combination with SLURM. Podman can not be successfully updated to a more recent version from what is bundled with centos7. Because of this there are issues relating to the queueing of jobs. When a resource becomes available podman jobs all attempt to start at the same time and conflict with each other and don't start. However this is unlikely to be an issue on ubuntu where we can update podman to a version where this bug has been resolved.
...
Alternatively another container option is Singularity. This is supported on NOAAs HPC and cloud mounted volumes. Docker/Podman utilizes a Dockerfile from which an image is built, Singularity has a recipe file from which an image is built. A singularity image has an `.sif` extension and this file and can be stored anywhere. 

You will find the file `atlantis6536.sif` file and the `Singularity` recipe file in the \\contrib\\atlantisCode` folder

-   `sudo singularity exec --bind /contrib/neus-atlantis/currentVersion:/app/model,/contrib/Andrew.Beet/out1:/app/model/output /contrib/atlantisCode/atlantis6536.sif` (By default this will run `RunAtlantis.sh` found in currentVersion)
...
-   Create a file with the .sh extension (for this example, job.sh) and copy the following
    ...
    > #!/bin/bash
    >
    > sudo singularity exec --bind /contrib/neus-atlantis/currentVersion:/app/model,/contrib/Andrew.Beet/out1:/app/model/output /contrib/atlantisCode/atlantis6536.sif 
...
### Practical use of sbatch {#sbatch}
...
-   Create a `job.sh` for each run that will execute the `RunAtlantis.sh`. The `job.sh` will have the form
    ...
    > #!/bin/bash
    >
    > sudo singularity exec --bind /contrib/neus-atlantis/currentVersion:/app/model,/contrib/Andrew.Beet/out1:/app/model/output /contrib/atlantisCode/atlantis6536.sif /app/model/RunAtlantis1.sh
...
for (ifolder in 1:nfolders) {
  ...
  folder <- paste0("/contrib/Andrew.Beet/out",ifolder)
  if (!dir.exists(folder)){
    dir.create(paste0("/contrib/Andrew.Beet/out",ifolder))
  }
  ...
  filenmjob <- paste0("/contrib/Andrew.Beet/job",ifolder,".sh")
  fileConn2<-file(filenmjob,open="w")
  cat("#!/bin/bash\n",file=fileConn2,append=T)
  runString <- paste0("sudo singularity exec --bind /contrib/neus-atlantis/currentVersion:/app/model,/contrib/Andrew.Beet/out",ifolder,":/app/model/output /contrib/atlantisCode/atlantis6536.sif /app/model/RunAtlantis",ifolder,".sh") 
  ...
  # submit job to slurm
  batchString <- paste0("sbatch -N 1 /contrib/Andrew.Beet/job",ifolder,".sh")
  system(batchString)
