...
Objective 3 addresses a possibility that similar NEUSv2 hindcast results could be obtained with any catch forcing with a similar mean and variance. However, if NEUSv2 were highly sensitive to the order of historical events, it would improve confidence that the NEUSv2 hindcast was capturing a specific regional signature (i.e. path dependence). Alternative catch forcing scenarios were constructed to determine whether the final model state was sensitive to the ordering of historical events. These scenarios simulated the model unaltered for the first 20 years to allow the model to spin up, then generating a new catch forcing by sampling without replacement the annual catch forcing for the remaining years (1985-2021), followed by a 5 year projection at contemporary catch levels. This allowed each NEUSv2 scenario to experience an alternative ordering of historical fishing with a common 5-year period over which to compare. By permuting the catch history instead of sampling with replacement, we ensure that the total harvest from the ecosystem remains constant between scenarios. A total of 750 permutation scenarios were performed. Then the mean biomass ($B_{i,s}$) for each species (s) was calculated over the last 5 years for each permutation scenario (i). Then the 95% credible interval was calculated for all $B_{i,s}$. If the historical simulation biomass for that species ($B_{s}'$) lies outside that interval, we considered species $s$ to be strongly path dependent. For those cases where $B_{i,s}$ did not lie outside the randomized interval, a bootstrapped (n = 10,000) confidence interval of the median of $B_{i,s}$ to test if $B_{s}'$ was significantly different than the median of the permutation scenarios. If $B_{s}'$ was outside this confidence interval, we considered species $s$ to be weakly path dependent.
...
The current iteration of NEUSv2 meets these baseline calibration criteria for the simulated hindcast. Furthermore, the sensitivity analyses presented here indicate that the NEUSv2 simulated functional groups are respond nonlinearly to changes in harvest levels, can recover from mild to moderate disturbances, and their contemporary biomass is contingent on the ordering of historical fishing pressure. Ecosystem models that lack path dependent process pose the risk of simulating a plausible variation of the system that does not contain the ecological memory of the real ecosystem. Subsequent work will be done to explore these path dependent processes more thoroughly, as this appears to be lacking in the ecosystem modelling literature. As ecosystem models are sought to inform pressing resource management problems, it is important that they are adequately evaluated for their process sensitivity not just in parameter choices. This is particularly important when simulating the bio-economic interations between fisheries adaptation strategies, changing resources, and broader environmental change.
