...
lastyrs <- longatBtxttidy %>%
  filter(Time %in% seq(startlast*365, floor(longrunpar$nyears)*365, by=365)) %>%
  group_by(species) %>%
  rename(lastbio = biomass)

#this isn't showing the geom_gls line on my work computer that showed on my laptop, code should be identical--confirmed works with short test dataset on laptop.
#also, geom_gls errors out when I include the long run as input. not sure why

#stable <- atBtxttidy %>%
#  filter(Time %in% seq((floor(runpar$nyears)-nlast)*365, floor(runpar$nyears)*365, by=365)) %>%
stable <- longatBtxttidy %>%
  ...
  #geom_point() +
  ggthemes::theme_tufte() +
  #ggplot(aes(yr, biomass)) +   geom_point() + ggthemes::theme_tufte() +
  geom_line(data=longatBtxttidy, aes(x=yr, y=biomass)) +
  #geom_point(data=lastyrs, aes(x=yr, y=lastbio, color="last")) #+
  geom_gls(warn = FALSE)
